...
out_sumr$treament_general <- ifelse(grepl("Clean", out_sumr$treatment), "Clean Plastic", 
                                    ifelse(grepl("odor", out_sumr$treatment), "Krill Odor",
                                           ifelse(grepl("Bf", out_sumr$treatment), "Biofouled Plastic", 
                                                  NA)))

## Order
out_sumr$treament_general <- ordered(out_sumr$treament_general, levels = c("Clean Plastic", "Biofouled Plastic", "Krill Odor"))
...
(ts.conc <- ggplot(data = out_sumr[is.na(out_sumr$treament_general) == F & out_sumr$time2 < 600,],
   ...
   geom_smooth(data = filter(out_sumr[is.na(out_sumr$treament_general) == F & out_sumr$time2 < 600,], time2 < 0), se = T, span = 0.9, colour = "black") +
   geom_smooth(data = filter(out_sumr[is.na(out_sumr$treament_general) == F & out_sumr$time2 < 600,], time2 > 0), se = T, span = 0.9, colour = "black") +
   geom_vline(xintercept = 0, linetype="dashed") +
   ylim(-5, 30) +
   scale_x_continuous(breaks = seq(-120, 600, by = 120)) + 
   labs(x = "Time (seconds) relative to treatment presentation",
        y = "Standardized Clustering \n Metric (Moran's I)") +
   scale_color_manual(name = "Treatment", values = c("khaki4", "seagreen3", "darksalmon")) +
   theme_bw() +
...
(ts.sw.food <- ggplot(data = out_sumr[out_sumr$treatment == c("Seawater Control", "Food") & out_sumr$time2 < 600,],
    ...
    geom_point(alpha = 1/5) +
    geom_smooth(data = filter(out_sumr[out_sumr$treatment == c("Seawater Control", "Food") & out_sumr$time2 < 600,], time2 < 0), se = T, span = 0.9, colour = "black") +
    geom_smooth(data = filter(out_sumr[out_sumr$treatment == c("Seawater Control", "Food") & out_sumr$time2 < 600,], time2 > 0), se = T, span = 0.9, colour = "black") +
    geom_vline(xintercept = 0, linetype="dashed") +
    ylim(-5, 30) +
    scale_x_continuous(breaks = seq(-120, 600, by = 120)) + 
...
(rh.conc <- ggplot(data = rh[is.na(rh$treament_general) == F,],
                   aes(x= time_adj, y = Proportion.fish.non.rheotactic, colour = as.factor(treament_general))) +
    geom_point(alpha = 1/5) +
    geom_smooth(data = filter(rh[is.na(rh$treament_general) == F,], time_adj >= 0), se = T, span = 0.9, colour = "black") +
    geom_smooth(data = filter(rh[is.na(rh$treament_general) == F,], time_adj <= 0), se = T, span = 0.9, colour = "black") +
    geom_vline(xintercept = 0, linetype="dashed") +
    ylim(0, 1) +
    labs(x = "Time (seconds) relative to treatment presentation",
    ...
    theme(plot.title = element_text(face="bold",vjust=30, size = 18),
          ...
          legend.position = "none") + 
    facet_grid(concentration ~ treament_general, scales = "free_x") + 
    annotate("text", x = -70, y = 0.90, label = c("C", "D", "E", "", "", "", "", "", ""), fontface = 2))

## Graph of the time series for SW control and food
(rh.sw.food <- ggplot(data = rh[rh$treatment == c("Seawater Control", "Food") ,],
                      aes(x= time_adj, y = Proportion.fish.non.rheotactic, colour = as.factor(treatment))) +
    geom_point(alpha = 1/5) +
    geom_smooth(data = filter(rh[rh$treatment == c("Seawater Control", "Food"),], time_adj >= 0), se = T, span = 0.9, colour = "black") +
    geom_smooth(data = filter(rh[rh$treatment == c("Seawater Control", "Food"),], time_adj <= 0), se = T, span = 0.9, colour = "black") +
    geom_vline(xintercept = 0, linetype="dashed") +
    ylim(0, 1) +
    scale_x_continuous(breaks = times) +
    ...
    theme(plot.title = element_text(face="bold",vjust=30, size = 18),
          ...
          legend.position = "none") +
    facet_grid(. ~ treatment, scales = "free_x", drop = T) +
    annotate("text", x = -70, y =0.90, label = c("A", "B"), fontface = 2))
...
## Save
ggsave(filename = "/Users/Tyson/Google Drive/Anchovy feeding videos/Analysis/figures/non_rheotactic_timeseries.pdf",
       plot = mp.fig, 
       width=174,
       height=174,
       dpi = 300,
